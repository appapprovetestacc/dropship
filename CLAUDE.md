# dropship — Page Builder Lite (Shogun variant)

> Hand-built Shopify app, delivered ready-to-iterate. Live at https://dropship.appapprove.app
>
> **Project facts:**
> - Blueprint: Page Builder Lite
> - Variant: **Shogun** — Like Shogun — clean library of pre-built sections.
...
A PageFly-class page builder — template-driven, not drag-and-drop canvas. Pick from 6 templates (Landing / About / Contact / Product Spotlight / FAQ / Coming Soon), edit text + images + CTAs via a clean block list with reordering, preview mobile + desktop, auto-resize uploaded images to 320/640/1280, publish as a Shopify Page (Online Store 2.0). Edit + republish anytime. Per-page SEO meta-fields.

**Shipped surfaces:**
- 6 templates: Landing, About, Contact, Product Spotlight, FAQ, Coming Soon
- Block-list editor with drag-handle reordering (no canvas)
- Mobile + desktop preview toggle
- Auto image-CDN resize on upload (320 / 640 / 1280 px)
- Publish-to-Shopify-Page (Online Store 2.0)
- Per-page SEO fields: title, description, og:image, canonical
- Edit + republish anytime, with last-10 revision history

## Iteration cookbook

Open this repo in Claude Code or Cursor and try one of these:

- *"add a video-block type to the block library"*
- *"add A/B testing for published landing pages (50/50 split)"*
- *"add a CTA-button block with target-URL + style preset"*
- *"add a 'share preview link' button per draft page"*
- *"add an SEO-score check on save (title length, meta description, og:image)"*
...
→ `deploy.yml` runs install + tsc + test + `wrangler deploy`. Live URL is `dropship.appapprove.app`.
